...
class ChannelPager(ActionBase):
    ...
    def _handle_tap(self) -> None:
        # One press can reach here multiple times (on_key_down + event_callback,
        # and once per instance when the page double-loads). The backend
        # debounces toggle_mute per user, so the extra calls are no-ops there.
